...
export class PreviewWeb {
  ...
  async onUpdateArgs({
    ...
    await Promise.all(this.storyRenders.filter(r => r.id === storyId).map(r => r.rerender())); // NOTE: we aren't checking to see the story args are targetted at the "right" story.
  ...
  async onResetArgs({
    ...
    const {
      initialArgs
    } = storyId === ((_this$currentRender2 = this.currentRender) === null || _this$currentRender2 === void 0 ? void 0 : _this$currentRender2.id) ? this.currentRender.story : await this.storyStore.loadStory({
      storyId
    });
  ...
  async renderSelection({
    ...
    const storyRender = new StoryRender(this.channel, this.storyStore, this.renderToDOM, this.mainStoryCallbacks(storyId), storyId, 'story'); // We need to store this right away, so if the story changes during
