...
const buildArgsParam = (initialArgs, args) => {
  ...
  return lib.stringify(encodeSpecialValues(object), QS_OPTIONS).replace(/ /g, '+').split(';').map(part => part.replace('=', ':')).join(';');
};

const queryFromString = memoize(1000)(s => lib.parse(s, {
  ignoreQueryPrefix: true
}));

const queryFromLocation = location => queryFromString(location.search);

const stringifyQuery = query => lib.stringify(query, {
  addQueryPrefix: true,
  encode: false
});
